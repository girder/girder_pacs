    ...
    v-flex(xs12, md10, offset-md1)
      v-layout(row, wrap)
        ...
        v-flex.mt-3.px-3(xs12, md6)
          v-text-field(placeholder="Search...", prepend-icon="mdi-magnify", v-model="search")
          v-layout(row)
            v-flex(xs6)
              v-menu(lazy, :close-on-content-click="false", v-model="startDateMenu", offset-y,
                ...
                v-text-field(label="Start date", prepend-icon="mdi-calendar-range",
                    slot="activator", v-model="startDate")
            ...
            v-flex.pl-3(xs6)
              v-menu(lazy, :close-on-content-click="false", v-model="endDateMenu", offset-y,
                ...
                v-text-field(label="End date", prepend-icon="mdi-calendar-range", slot="activator",
                    v-model="endDate")
...
import { pagingContainer } from '@/utils/mixins';
import Series from './Series.vue';

export default {
  components: { Series },
  mixins: [pagingContainer],
  inject: ['girderRest'],
  data: () => ({
    studies: [],
    series: [],
